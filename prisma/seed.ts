...
const COURSES = [
    ...
    { title: "Desarrollo de Inteligencia Emocional", slug: "inteligencia-emotional", students: 124, progress: 55, pendingEval: 18, inquiries: 14, status: "Activo" },
...
async function main() {
    ...
    // 0. Clean previous data (to avoid duplication on specific IDs if any)
    ...
    // 2. Setup Primary Teacher for Demo (profe@empresa.com)
    const teacherEmail = 'profe@empresa.com'
    ...
    // Setup Super Admin just in case
    ...
    for (const c of COURSES) {
        const course = await prisma.course.create({
            data: {
                title: c.title,
                slug: c.slug,
                description: `Curso completo de ${c.title}. Datos simulados para demostración premium.`,
                category: "Liderazgo",
                duration: 40,
                level: "INTERMEDIATE",
                published: true,
        ...
        for (let i = 1; i <= 5; i++) {
            const module = await prisma.module.create({
                data: {
                    title: `Módulo ${i}: Fundamentos de ${c.title}`,
                    order: i,
                    courseId: course.id
                }
            })
            const lesson = await prisma.lesson.create({
                data: {
                    ...
                    order: 1,
                    moduleId: module.id,
                    content: "Contenido extendido para la lección de prueba. En esta sección analizaremos los pilares fundamentales...",
                    contentType: "VIDEO",
            ...
            // Evaluation (simulate pending stuff for teacher)
            if (c.pendingEval > 0 && i === 4) {
                const evaluation = await prisma.evaluation.create({
                    data: {
                        ...
                        questions: {
                            create: [{
                                question: "¿Cómo aplicarías esto a tu equipo?",
                                type: "OPEN",
                                options: "[]",
                                correctAnswer: "",
                                order: 1
                ...
                // Create submissions to match Category 3
                for (let j = 0; j < Math.min(c.pendingEval, 5); j++) {
                    await prisma.evaluationSubmission.create({
                        data: {
                            evaluationId: evaluation.id,
                            userId: teacherUser.id, // using teacher as student for pending stuff visualization
                            answers: "Esta es una entrega simulada para calificar.",
                            score: 0,
                            maxScore: 100,
                            percentage: 0,
                            passed: false,
        ...
        for (let j = 0; j < c.inquiries; j++) {
            await prisma.inquiry.create({
                data: {
                    ...
                    courseId: course.id,
                    teacherId: teacherUser.id,
                    question: `Duda sobre ${c.title}. ¿Podría profundizar en el módulo ${Math.floor(Math.random() * 5) + 1}?`,
                    status: "PENDING",
    ...
    const colaboradorEmail = 'colaborador@empresa.com'
    const colaboradorPassword = await bcrypt.hash('123456', 10)
    const colaboradorStudent = await prisma.user.upsert({
        where: { email: colaboradorEmail },
        update: {
            role: 'STUDENT',
            password: colaboradorPassword,
            name: 'Estudiante Prueba (Collab)'
        },
        create: {
            ...
            password: colaboradorPassword,
            companyId: mainCompany.id,
            avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=collab`
        }
    })

    // Enroll Colaborador in first 5 courses
    const allCourseIds = Array.from(courseMap.values())
    for (let i = 0; i < 5; i++) {
        await prisma.enrollment.upsert({
            where: { userId_courseId: { userId: colaboradorStudent.id, courseId: allCourseIds[i] } },
            update: { progress: 85, status: 'ACTIVE' },
            create: {
                userId: colaboradorStudent.id,
    ...
    const featuredUserMap = new Map()
    const studentPassword = await bcrypt.hash('student123', 10)
    for (const s of FEATURED_STUDENTS) {
        const email = `${s.name.toLowerCase().replace(/ /g, '.')}@${s.companySlug}.com`
        const user = await prisma.user.upsert({
            where: { email },
            update: { avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${s.name}`, password: studentPassword },
            create: {
                name: s.name,
                email,
                role: 'STUDENT',
        ...
        featuredUserMap.set(s.name, user.id)
        ...
        if (courseId) {
            ...
            if (s.progress > 0) {
                const lessons = await prisma.lesson.findMany({
                    ...
                    take: Math.floor((s.progress / 100) * 5)
                })
            ...
            // Diagnostic Result (Categoría 2 scores)
            // Note: Diagnostic model needs to exist first if using it, let's just use enrollment score if possible or submissons
            ...
            if (s.progress === 100) {
                await prisma.certificate.upsert({
                    ...
                    create: {
                        userId: user.id,
                        courseId,
                        code: `CERT-2026-${Math.floor(Math.random() * 9000) + 1000}`,
